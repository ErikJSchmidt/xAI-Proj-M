...
class Skipped18LayerForLowDimEmbbeding(nn.Module):
    """"
    ...
    Separated the CNN and the fully connected layer into different sequentials, so that the FC can just be dropped on a
    fitted model to obtain embeddings instead of predictions.

    Make embeddings only 2-dimensional so we can easily visualize it
    """
    def __init__(self):
        super().__init__()
        self.network = nn.Sequential(
            ...
            nnm.ModularSkip(
                nn.Sequential(
                    ...
                    nn.BatchNorm2d(128),
                    nn.ReLU(),
                    nn.Conv2d(128, 2, kernel_size=3, padding=1, bias=False),  # 128*128*3*3 = 147456
                )
            ),
            nn.BatchNorm2d(2),
            nn.ReLU(),# --------------------
